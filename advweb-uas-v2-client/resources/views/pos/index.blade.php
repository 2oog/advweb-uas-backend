...
<script>
    ...
    const apiUrl = "{{ $apiUrl }}";
    const serverUrl = "{{ config('api.server_url') }}";
    ...
    function renderMenu() {
        ...
        grid.innerHTML = menuItems.map(item => {
            let imageSrc = item.image_asset;
            if (imageSrc && !imageSrc.startsWith('http')) {
                // Ensure no double slashes if serverUrl ends with / or imageAsset starts with /
                const baseUrl = serverUrl.replace(/\/$/, '');
                const path = imageSrc.replace(/^\//, '');
                imageSrc = `${baseUrl}/${path}`;
            }

            const isUrl = !!item.image_asset;
            const imageHtml = isUrl 
                ? `<img src="${imageSrc}" class="w-100 h-100 rounded" style="object-fit: cover;" alt="${item.name}" onerror="this.onerror=null;this.innerHTML='<i class=\'fas fa-utensils\'></i>';this.className='d-none';this.parentNode.innerHTML='<i class=\'fas fa-utensils fa-2x text-muted\'></i>';">`
    ...
    // --- INIT ---
    renderMenu(); // Menu data pre-loaded from controller via Guzzle
    updateMobileBadge(0); 
</script>
<!-- Toast Container -->
